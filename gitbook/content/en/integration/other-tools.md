...
ToolNet API is compatible with any tool that supports the OpenAI API format. This guide covers generic integration patterns for various tools and custom applications.

## Overview

ToolNet API provides an OpenAI-compatible API endpoint that works with:
...
Any OpenAI-compatible tool can connect to ToolNet API using these settings:
...
**Problem:** Cannot connect to ToolNet API
```bash
# Check if ToolNet API is running
...
- Check network connection to ToolNet API

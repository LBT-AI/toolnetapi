...
Tích hợp ToolNet API với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của ToolNet API.
...
ToolNet API cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong ToolNet API dashboard
...
1. Xác minh tên model khớp với cấu hình ToolNet API
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong ToolNet API dashboard
...
Đảm bảo bạn đã cấu hình API key trong ToolNet API cloud dashboard.

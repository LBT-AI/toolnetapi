...
const RESET_HINT = "Forgot password? Reset to default via ToolNet API CLI → Settings → Reset Password to Default.";
...
export async function POST(request) {
  try {
    ...
    if (isValid) {
      ...
      // If user is logging in with default password (no custom password set in database yet),
      // force changing the password immediately.
      const mustChangePassword = !storedHash;

      if (mustChangePassword) {
        return NextResponse.json(
          { success: true, mustChangePassword: true, message: "Default password in use. Please set a new password." },

...
Despliega ToolNet API en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.toolnetapi` | Ruta de almacenamiento de la base de datos |
...
Crea `/etc/nginx/sites-available/toolnetapi`:
...
# Si NO usas proxy reverso, permite los puertos de ToolNet API
...
```bash
# Verificar si ToolNet API está corriendo
pm2 status

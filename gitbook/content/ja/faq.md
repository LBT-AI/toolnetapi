...
ToolNet APIに関する一般的な質問。

---

## ToolNet APIとは?

**ToolNet APIは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、ToolNet API自体は100%無料でオープンソースです。**
...
**ToolNet APIはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## ToolNet APIをセルフホストできますか?

**はい! ToolNet APIは複数のデプロイメントオプションをサポートします:**
...
- `DATA_DIR` - データベース保存パス(デフォルト: `~/.toolnetapi`)
...
**はい、ToolNet APIはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.toolnetapi`(またはカスタム`DATA_DIR`)にローカル保存
- ToolNet APIサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化

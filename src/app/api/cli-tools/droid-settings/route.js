...
// POST - Update ToolNet API customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing ToolNet API configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:ToolNet API"));
...
// DELETE - Remove ToolNet API customModels only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove ToolNet API customModels
    if (settings.customModels) {
      settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:ToolNet API"));

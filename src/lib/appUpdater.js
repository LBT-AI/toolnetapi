  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "toolnetapi")
        : path.join(os.homedir(), ".toolnetapi"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all toolnetapi-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from toolnetapi install dir or wrapper cli.js
        const isAppProcess = lower.includes("toolnetapi") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("toolnetapi") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "toolnetapi");
  }
  return path.join(os.homedir(), ".toolnetapi");
}
...
// Resolve npx/toolnetapi binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx toolnetapi` — works regardless of global bin path changes after npm i -g

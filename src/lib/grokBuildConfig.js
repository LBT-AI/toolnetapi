...
const previousDefaultRegExp = /^# toolnetapi-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# toolnetapi-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via ToolNet API gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# toolnetapi-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# toolnetapi-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}

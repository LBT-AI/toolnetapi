...
// POST - Apply ToolNet API config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing ToolNet API entry or append
    const idx = config.findIndex((e) => e.name === "ToolNet API");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove ToolNet API entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "ToolNet API");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "ToolNet API removed from Copilot config",
    });
  } catch (error) {

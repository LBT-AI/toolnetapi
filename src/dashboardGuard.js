...
const ALWAYS_PROTECTED = [
  ...
  "/api/oauth/freebuff/auto-import",
];
...
function isLoopbackPeer(request) {
  if (hasTrustedPeerHeaders(request)) {
    return isLoopbackHostname(request.headers.get("x-toolnet-real-ip") || request.headers.get("x-9r-real-ip"));
  }
...
export function isLocalRequest(request) {
  ...
  if (request.headers.get("x-toolnet-via-proxy") || request.headers.get("x-9r-via-proxy")) return false;

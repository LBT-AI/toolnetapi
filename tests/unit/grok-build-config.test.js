...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("toolnetapi");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "toolnetapi-general-purpose",
      explore: "toolnetapi-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.toolnetapi\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.toolnetapi-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.toolnetapi-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# toolnetapi-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.toolnetapi-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("toolnetapi-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.toolnetapi-");
    expect(reset).not.toContain("toolnetapi-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("toolnetapi-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("toolnetapi-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("toolnetapi-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("toolnetapi-explore");
    expect(getGrokSubagentSlot("plan")).toBe("toolnetapi-plan");

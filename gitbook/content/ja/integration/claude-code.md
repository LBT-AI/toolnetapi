...
ToolNet APIをClaude Code CLIと統合し、AnthropicのAPIリクエストをToolNet APIのインテリジェントルーティングシステム経由でルーティングします。
...
Claude CodeはToolNet APIモデルにマッピングされる以下のモデルエイリアスをサポート:
...
1. モデル名がToolNet APIの設定と一致しているか確認

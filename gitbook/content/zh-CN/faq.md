...
关于 ToolNet API 的常见问题。

---

## 什么是 ToolNet API?

**ToolNet API 是一款 AI 模型路由工具,能够最大化你的订阅价值并最小化成本。**
...
**环境变量:**
- `JWT_SECRET` - **生产环境必须修改!**
- `DATA_DIR` - 数据库存储路径(默认:`~/.toolnetapi`)
...
**是的,ToolNet API 优先考虑安全和隐私:**

**本地存储:**
- 所有数据存储在本地 `~/.toolnetapi`(或自定义 `DATA_DIR`)
- 不会发送数据到 ToolNet API 服务器

...
async function buildDroidHeader() {
  ...
  // Extract ToolNet API custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:ToolNet API-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract ToolNet API provider config
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use ToolNet API\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",

...
const hasToolNetAPIConfig = (settings) => {
  ...
  return !!settings.models.providers["toolnetapi"];
};

// Read per-agent models.json and return current model id (without "toolnetapi/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["toolnetapi"]?.models;
...
// POST - Update ToolNet API settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `toolnetapi/${model}`;

    // Remove all old toolnetapi/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("toolnetapi/"))
    ...
    // Add fresh toolnetapi models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`toolnetapi/${m}`] = {};
    });

    // Remove old toolnetapi model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("toolnetapi/")) {
    ...
    // Update models.providers.toolnetapi with all models
    settings.models.providers["toolnetapi"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `toolnetapi/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove ToolNet API settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove ToolNet API from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["toolnetapi"];
    ...
    // Remove toolnetapi models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("toolnetapi/"));
    ...
    // Reset agents.defaults.model.primary if it uses toolnetapi
    if (settings.agents?.defaults?.model?.primary?.startsWith("toolnetapi/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "ToolNet API settings removed successfully",
    });
  } catch (error) {

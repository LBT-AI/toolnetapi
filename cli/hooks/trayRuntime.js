...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.toolnetapi/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/toolnetapi/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[toolnetapi][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[toolnetapi][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[toolnetapi][runtime] chmod tray bin failed: ${e.message}`);
  }
}

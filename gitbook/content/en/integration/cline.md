...
Integrate ToolNet API with Cline VSCode extension to route your AI requests through ToolNet API's intelligent routing system.

## Prerequisites

- Visual Studio Code installed
- Cline extension installed from VSCode marketplace
- ToolNet API running locally or cloud endpoint configured
- API key from ToolNet API dashboard
...
Set the base URL to your ToolNet API endpoint:

**For Local ToolNet API:**
```
...
**Steps:**
1. In the **Base URL** field, enter your ToolNet API endpoint
2. Make sure to include `/v1` at the end

### 4. Add API Key

1. In the **API Key** field, enter your ToolNet API API key
2. You can find your API key in the ToolNet API dashboard under **Settings → API Keys**
3. The key should start with `sk-toolnetapi-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your ToolNet API configuration
...
You can use any model configured in your ToolNet API dashboard. Common examples:
...
4. Cline will use ToolNet API to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using ToolNet API
...
3. Get AI-powered explanations through ToolNet API

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use ToolNet API to understand context and make changes
...
1. Verify ToolNet API is running: `curl http://localhost:20128/health`
...
1. Verify the model name matches exactly with your ToolNet API configuration
2. Check that the provider connection is active in ToolNet API dashboard
...
2. Verify your ToolNet API instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check ToolNet API logs for any errors
...
1. In Cline settings, set Base URL to: `https://toolnetapi.com`
2. Make sure you have configured your API key in the ToolNet API cloud dashboard
...
2. **Monitor Usage**: Check ToolNet API dashboard for usage statistics and costs
...
## Integration with ToolNet API Features

### Model Routing

ToolNet API automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, ToolNet API automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through ToolNet API dashboard:
- Total requests
- Token usage
- Cost per model

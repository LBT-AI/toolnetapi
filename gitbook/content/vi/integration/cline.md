...
Tích hợp ToolNet API với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của ToolNet API.

## Yêu cầu

- Visual Studio Code đã cài đặt
- Extension Cline đã cài đặt từ VSCode marketplace
- ToolNet API đang chạy cục bộ hoặc cloud endpoint đã cấu hình
- API key từ ToolNet API dashboard
...
Đặt base URL tới endpoint ToolNet API:

**Cho ToolNet API cục bộ:**
```
...
**Các bước:**
1. Trong field **Base URL**, nhập endpoint ToolNet API
...
1. Trong dropdown **Model**, bạn có thể:
   ...
   - Nhập tên model thủ công từ cấu hình ToolNet API
...
Bạn có thể dùng bất kỳ model nào đã cấu hình trong ToolNet API dashboard. Ví dụ phổ biến:
...
3. Nhấn Enter để gửi
4. Cline sẽ dùng ToolNet API để xử lý request

### Tạo Code

1. Yêu cầu Cline tạo code: "Create a React component for a login form"
2. Cline sẽ tạo code qua ToolNet API
...
3. Nhận giải thích AI qua ToolNet API

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng ToolNet API để hiểu context và thực hiện thay đổi
...
1. Xác minh ToolNet API đang chạy: `curl http://localhost:20128/health`
...
1. Xác minh tên model khớp chính xác với cấu hình ToolNet API
2. Kiểm tra kết nối provider đang hoạt động trong ToolNet API dashboard
...
2. Xác minh ToolNet API instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs ToolNet API để xem lỗi
...
2. **Theo dõi Usage**: Kiểm tra ToolNet API dashboard để xem thống kê và chi phí
...
## Tích hợp với Tính năng ToolNet API

### Định tuyến Model

ToolNet API tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, ToolNet API tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua ToolNet API dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider

...
Tích hợp ToolNet API với Cursor IDE để định tuyến request AI qua hệ thống routing thông minh của ToolNet API.

## Yêu cầu

- Cursor IDE đã cài đặt
- Tài khoản Cursor Pro (cần thiết cho custom API endpoint)
- ToolNet API cloud endpoint đã cấu hình
...
1. Trong field **API Key**, nhập API key ToolNet API
...
3. Nhập tên model từ cấu hình ToolNet API (ví dụ: `gpt-4`, `claude-opus-4-5`, v.v.)
4. Click **Add**
...
3. Bắt đầu dùng ToolNet API với Cursor!
...
3. Bắt đầu chat với AI qua ToolNet API
...
2. Nhấn Cmd/Ctrl + K
3. Nhập prompt
4. Cursor sẽ dùng ToolNet API để tạo code
...
4. Nhận giải thích AI qua ToolNet API
...
Nếu bạn chạy ToolNet API cục bộ và muốn dùng với Cursor:

1. Bật cloud endpoint trong ToolNet API settings
2. Cấu hình URL cloud endpoint trong ToolNet API dashboard
3. Dùng URL cloud trong Cursor settings
4. Đảm bảo ToolNet API instance cục bộ có thể truy cập từ internet

## Best Practices

1. **Dùng Model Aliases**: Tạo alias ngắn cho model thường dùng trong ToolNet API

...
import { resolveGroqModels } from "open-sse/services/groqModels.js";
...
const LIVE_MODEL_RESOLVERS = {
  ...
  groq: async (conn) => {
    const proxy = await resolveConnectionProxyConfig(conn.providerSpecificData || {});
    const result = await resolveGroqModels(conn, {
      log: console,
      proxyOptions: {
        connectionProxyEnabled: proxy.connectionProxyEnabled === true,
        connectionProxyUrl: proxy.connectionProxyUrl || "",
...
// and break recursive loops between toolnetapi instances connected to each other.
...
export async function buildModelsList(kindFilter, options = {}) {
  ...
  // toolnetapi instance's fetchCompatibleModelIds — skip dynamic fetch to break
...
export async function GET(request) {
  try {
    // Detect cross-instance recursive /models fetch (another toolnetapi fetching our /models)

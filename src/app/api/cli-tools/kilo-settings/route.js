...
const hasToolNet APIConfig = (auth) => {
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasToolNet API: hasToolNet APIConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "ToolNet API", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "ToolNet API settings removed from Kilo Code" });
  } catch (error) {

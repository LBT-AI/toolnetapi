...
for (const entry of REGISTRY) {
  ...
  if (entry.models !== undefined) {
    const normalizedModels = entry.models.map(normalizeModel);
    PROVIDER_MODELS[entry.id] = normalizedModels;
    if (entry.alias) PROVIDER_MODELS[entry.alias] = normalizedModels;
    for (const a of entry.aliases || []) PROVIDER_MODELS[a] = normalizedModels;
  }

...
Combos are **custom fallback chains** that you create in the dashboard. Instead of using a single model, you define a sequence of models that ToolNet API tries in order.
...
ToolNet API automatically tries each model in sequence until one succeeds.
...
When limit reached, ToolNet API skips paid models and uses free tier only.

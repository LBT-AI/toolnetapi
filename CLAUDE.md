...
- The **CLI launcher** (`cli/`, published to npm as `toolnetapi`) — a separate package that installs/starts the server and manages the tray. It has its own `package.json`, version, and build.
...
- DB file location resolves via `src/lib/db/paths.js` (`DATA_DIR`, else `~/.toolnetapi/`).
- Usage/logs (`src/lib/usageDb.js`, `usage.json` + `log.txt`) still live under `~/.toolnetapi` and do **not** follow `DATA_DIR`.
